...
class FG_eval {
 public:
  ...
  // `vars` is a vector containing the variable values (state & actuators).
    ...
    // The part of the cost based on the reference state - cte, epsi and vel
    for (int i = 0; i < N; i++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double epsi = state[5];
	...
	//incorporate 100ms latency. Predict state after latency and pass that to the solver
  ...
  // The x, y from the solver are stored in MPC_x and MPC_y and passed to the simulator for plotting 
	this->mpc_x = {};
	this->mpc_y = {};
	for (int i = 0; i < N-1;i++){
  ...
  return {solution.x[delta_start],   solution.x[a_start] // Steering and acceleration which are later passed to the simulator
	};
